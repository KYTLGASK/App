    ...
    private void OnMouseDown()
    {
        ...
        if (selected.GetComponent<SelectedUnitMove>().isSelected )//if there was a unit selected
        {
            ...
            if (unitToMove != null) {//if unit to move is not null
                ...
                && !(unitToMove.GetComponent<BasicUnitProperties>().moved) && !transform.GetComponent<HighlightOnTouch>().isXTerrain) //or there is no unit on this tile and the unit doesn't have a parent(is not on a tile)   and it is not XTerrain                 //and unit's tile is in range of unit's speed
                {
