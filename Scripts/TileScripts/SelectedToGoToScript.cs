    ...
    private void OnMouseDown()
    {
        ...
        if (selected.GetComponent<SelectedUnitMove>().isSelected )//if there was a unit selected
        {
            ...
            if (unitToMove != null) {//if unit to move is not null
                ...
                ||(transform.childCount < 6 && transform.GetComponent<Distance>().InRange(unitToMove.GetComponent<BasicUnitProperties>().GetSpeed(), unitToMove.transform.parent.gameObject))) //or there is no unit on this tile and the unit doesn't have a parent(is not on a tile)                    //and unit's tile is in range of unit's speed
            {
